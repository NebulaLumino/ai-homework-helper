...
export default function Home() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/generate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ prompt: `Homework Helper: " +  + " | ".join([inputs["subject"], inputs["grade_level"], inputs["topic"]])` }) });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 via-gray-950 to-gray-900 text-gray-100 py-12 px-4">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-10"><h1 className="text-4xl font-bold text-emerald-400 mb-4">Homework Helper</h1><p className="text-gray-400">AI-powered family tool</p></div>
